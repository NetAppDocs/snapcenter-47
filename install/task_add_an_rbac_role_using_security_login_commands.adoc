...
NOTE: For consistency, the SnapCenter documentation refers to the roles as using privileges. The OnCommand System Manager GUI uses the term _attribute_ instead of _privilege_. When setting up ONTAP RBAC roles, both these terms mean the same thing.
